	...
	public class Building_ShipAirlock : Building_Door
	{
		...
		public override IEnumerable<FloatMenuOption> GetFloatMenuOptions(Pawn pawn)
		{
			...
			if (Faction != Faction.OfPlayer)
			{
				...
				{
					options.Add(new FloatMenuOption("SoS.Hack".Translate(), delegate { Job hackAirlock = new Job(ResourceBank.JobDefOf.HackAirlock, this); pawn.jobs.TryTakeOrderedJob(hackAirlock); }));
				}
				...
				{
					options.Add(new FloatMenuOption("SoS.AirlockBreach".Translate(), delegate { Job breachAirlock = new Job(ResourceBank.JobDefOf.BreachAirlock, this); pawn.jobs.TryTakeOrderedJob(breachAirlock); }));
				}
			}
			return options;
		}
		...
		public override void Tick()
		{
			...
			//Glow when opened
			if (OpenPct > 0 && ticks % 16 == 0 && Map.IsSpace() && Outerdoor())
